# MCP 客户端 Plan> 技术栈：Python 3.12+；使用 **官方 SDK** `mcp`（`pip install mcp` / `uv add mcp`，import 名 `mcp`）承载协议层（JSON-RPC 编解码、`initialize` 握手、stdio 与 Streamable HTTP 传输）。本章新增 **`koyocode.mcp` 子包** 与入口装配，**不改 tool / agent / tui / permission / llm / config / conversation / prompt**。

## 架构概览- **`koyocode.mcp` 子包（新增）**：承载 MCP 客户端的全部职责——配置加载与两层合并、`${VAR}` 展开、字段校验、调用 SDK 建立 stdio / HTTP 会话、把远端工具适配成内置 `Tool` 协议、统一管理生命周期。仅依赖 `koyocode.tool`、SDK 与标准库；不依赖 agent / tui / permission / conversation。
- **`koyocode.cli`（改造）**：在 `tool.default_registry()` 之后、`permission.PermissionEngine(...)` 与 `koyoCodeApp(...).run()` 之前，加载 mcp 配置 → 启动 Manager → 把 Manager 产出的工具注册进 registry → 退出时 `await manager.close()`（包在 `try/finally` 中）。
- **`koyocode.tool` 包（零改）**：`Registry.register` 与 `Tool` 协议本就是开放抽象，直接吃 `McpTool` 实例；`is_read_only` 对 MCP 工具返回正确值。
...
## 核心数据结构### `koyocode.mcp.Config` / `koyocode.mcp.ServerConfig`（对外）
```python
...
### 工具适配（包内私有）
```python
# McpTool 实现 koyocode.tool.Tool 协议（无参方法暴露，对齐内置工具）。
# 不用 @dataclass：Tool 协议要求 name()/description()/parameters()/read_only()
# 是方法，dataclass 字段与方法同名会冲突，故以 _前缀字段存值、方法返回。
class McpTool:
    def __init__(self, full_name: str, remote_name: str, description: str,
                 parameters: dict[str, Any], read_only: bool, caller: CallerSession) -> None:
        ...
        self._read_only = read_only        # 仅来自远端 annotations.readOnlyHint==True
...
# - root: 项目根（用来定位 <root>/.koyocode.yaml）
...
# version 透传到 Implementation.version（便于 server 端识别 koyocode 版本）。
...
## 模块设计### `src/koyocode/mcp/config.py`
...
- `async def _do_connect(mgr, name, srv, version)`：
  ...
  - 用一个**包级 `AsyncExitStack`**（挂在 `Manager._stack`）持有 transport 与 `ClientSession` 上下文：
    ...
    session = await mgr._stack.enter_async_context(
        ClientSession(read, write, client_info=Implementation(name="koyocode", version=version))
    )
    await session.initialize()                  # 握手
    listed = await session.list_tools()
    ```
  - 对 `listed.tools` 中每个 `Tool` 调 `adapt_tool(name, t, session)`；成功的入临时 list。
  - 在 `async with mgr._lock:` 内统一 append `_sessions` / `_tools`；**append 前按 `full_name` 去重**：若该名已存在于 `mgr._tools`（跨 server 同名边界），后到者跳过并 stderr 告警 `[mcp] warn: skip duplicate tool <full_name>`（不向 Registry 注册重名——`Registry.register` 遇重名抛 `ValueError` 会中断启动，故必须在 Manager 层消化）。
  - **`AsyncExitStack` 并发安全**：多个 `_connect_one` task 并发对同一 `mgr._stack` 调 `enter_async_context` 不安全（其内部列表非并发安全、`__aenter__` 有 await 点），故 `enter_async_context` 与随后的 append 必须在 `mgr._lock` 持有期间成对完成（lock 包住整个 enter+append），或每 task 用独立 `AsyncExitStack` 再合并。
...
### `src/koyocode/mcp/tool.py`
**职责：** 把 SDK 返回的 `mcp.types.Tool` 适配为 koyocode `Tool` 协议。
**关键点：**
...
- `McpTool.name / description / parameters / read_only`：通过 dataclass 字段直接暴露（koyocode `Tool` 协议要求的属性/方法返回字段值）。
- `async def McpTool.execute(self, args: str) -> ToolResult`（`args` 为 raw JSON 字符串，与内置工具签名一致；上层传 str 不是 dict）：
  - `arg_map = json.loads(args) if args and args.strip() else None`（空串视作无参数；`json.loads` 失败转 `is_error=True` 结构化错误回灌）；
  - ```python
    try:
...
async def _amain() -> int:
    ...
    try:
        ...
        app = koyoCodeApp(cfg.providers, registry=registry, engine=engine)
        await app.run_async()
    finally:
        await mcp_mgr.close()
    return 0
...
| 配置文件位置 | 项目级 `<root>/.koyocode.yaml` + 用户级 `~/.koyocode/config.yaml` | 用户拍板；注意现有 providers 配置是项目级 `<root>/.koyocode/config.yaml`（cwd 下、缺失即退出），MCP 用户级在 `~/.koyocode/config.yaml`（home 下）--两者路径不同（`./` vs `~/`）不冲突，但同名 `config.yaml` 易混，需知悉是两个文件 |
...
| 未定义变量 | 空串 + 一次性告警（不阻断） | server 自决无凭据时是否能跑；koyocode 不替它拍板 |
...
依赖方向（无环）：`koyocode.cli → koyocode.mcp → {koyocode.tool, mcp(SDK), 标准库}`；`koyocode.mcp` 不依赖 agent / tui / permission / conversation。