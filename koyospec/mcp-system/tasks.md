...
| 新建 | `src/koyocode/mcp/__init__.py` | 暴露 `Config` / `ServerConfig` / `Manager` / `load_config` / `new_manager` |
| 新建 | `src/koyocode/mcp/config.py` | `Config` / `ServerConfig`、`load_config`、`_load_file`、`_expand_vars`、`_apply_expansion`、`_merge_servers`、`_validate_server` |
| 新建 | `tests/test_mcp_config.py` | 两层合并 / `${VAR}` 展开 / 字段校验 / 降级 单测 |
| 新建 | `src/koyocode/mcp/tool.py` | `CallerSession` Protocol、`McpTool`、`adapt_tool`、`execute`、非 text 块告警 once set |
| 新建 | `tests/test_mcp_tool.py` | 命名拼接 / 禁用字符 / Execute 成功 / 远端 IsError / 超时 / 协议错 / 非 text 块跳过 单测 |
| 新建 | `src/koyocode/mcp/manager.py` | `Manager`、`_Session`、`new_manager`（`asyncio.gather` 并发 + 30s 超时）、`close`（5s 兜底）、`tools`；模块级 `connect_timeout` / `close_timeout` |
| 新建 | `tests/test_mcp_manager.py` | 连接成功 / 失败 / 超时、`close` 不死锁、并发写共享状态安全 单测 |
| 改   | `src/koyocode/cli.py` | 装配 `load_config`、`new_manager`、注册 MCP 工具、`finally: await mgr.close()` |
...
3. 写一段最小试导入（可直接放进后续 `tool.py` 的 import 中）：
   ...
   ```
   并在 Python REPL 跑一次 `import koyocode.mcp` 雏形，验证可用。
...
8. `load_config(root: str) -> Config`：
   - 用户级 = `Path.home() / ".koyocode" / "config.yaml"`（`Path.home()` 失败时跳过用户层不致错，用 `try/except` 兜底）；项目级 = `Path(root) / ".koyocode.yaml"`。
   - 两层各自 `_load_file`；返回空 dict 即视为该层为空。
   - 对每层各 server 跑 `_apply_expansion`。
   - `_merge_servers` 后逐个 `_validate_server`，收齐合法 server 组装 `Config`。
   - 永不抛出。
9. `src/koyocode/mcp/__init__.py` 中 `from .config import Config, ServerConfig, load_config`。

**验证：** `python -c "from koyocode.mcp import load_config, Config"` 不报错；`pytest tests/test_mcp_config.py` 覆盖：
...
3. 实现 koyocode `Tool` 协议要求的【无参方法】（协议要求方法不是字段）：`name()`（返回 `full_name`）、`description()`、`parameters()`、`read_only()`、`async def execute(args)`。用普通类或 `_` 前缀字段（如 `_name` / `_description` / `_parameters` / `_read_only`）存值，通过同名无参方法返回；dataclass 字段与方法同名会冲突，故不能用 dataclass 字段直接暴露。
...
5. `async def McpTool.execute(self, args: str) -> ToolResult`：
   - args 是 raw JSON 字符串（与内置工具签名一致；上层传 str 不是 dict），需 `json.loads` 解析为 dict 后再传给 `call_tool`；`arg_map = json.loads(args) if args and args.strip() else None`（空串视作无参数）；json.loads 失败转 `is_error=True` 结构化错误回灌。
   - ```python
     try:
...
5. `async def _do_connect(mgr, name, srv, version)`：
   ...
   - 再进入 `ClientSession(read, write, client_info=mtypes.Implementation(name="koyocode", version=version))` 上下文；
   - `await session.initialize()`；
   - `listed = await session.list_tools()`；
   - 对 `listed.tools` 调 `adapt_tool(name, t, session)`，收齐 list；
   - `async with mgr._lock:` 内 `mgr._sessions.append(_Session(name, session))`；对 `adapted` 逐个按 `full_name` 去重，已存在的 `full_name` 跳过并 stderr 告警 `[mcp] warn: skip duplicate tool <full_name>`，仅 extend 新工具。
   - **去重（必须）**：在 Manager 适配阶段按 `full_name` 去重，后到者跳过并 stderr 告警。不向 Registry 注册重名——`Registry.register` 遇重名抛 `ValueError` 会中断启动，故必须在 Manager 层消化。
   - **AsyncExitStack 并发安全**：多个 `_connect_one` task 并发对同一 `Manager._stack` 调 `enter_async_context` 不安全（其内部列表非并发安全、`__aenter__` 有 await 点）；需用 `mgr._lock` 包住整个 enter+append，或每 task 用独立 `AsyncExitStack` 再合并。
...
7. `async def Manager.close(self)`：
   ...
   ```
8. `src/koyocode/mcp/__init__.py` 追加 `from .manager import Manager, new_manager`、`from .tool import McpTool`。
...
3. 在 `registry = tool.default_registry()` 之后插入：
   ...
   try:
       for t in mgr.tools():
           registry.register(t)
       # 既有：构造 PermissionEngine、koyoCodeApp，await app.run_async()
       ...
   finally:
       await mgr.close()
   ```
...
1. 内容（用 YAML 注释说明放置位置与覆盖语义）：
   ```yaml
   # 项目级放 <root>/.koyocode.yaml；用户级放 ~/.koyocode/config.yaml。
