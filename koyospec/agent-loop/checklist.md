...
## 实现完整性
- [ ] 多轮自动连环：需要连续两步工具的任务，Agent 无需中途催促即自动多轮执行工具直到给出最终答复（验证：`python -m koyocode` 跑「读 A 文件 → 据内容新建 B 文件」，观察 `read_file` 与 `write_file` 跨多轮依次出现、最终答复）。(AC1/F1)
...
## 编译与测试
- [ ] `python -m koyocode` 能正常启动（在合法配置下进入 TUI）。
- [ ] `ruff check .` 无告警。
