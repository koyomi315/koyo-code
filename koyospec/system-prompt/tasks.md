...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/koyocode/prompt/modules.py` | `Module` 类型；`fixed_modules()` 七固定模块、`optional_modules()` 三空槽的内容常量 |
| 改   | `src/koyocode/prompt/__init__.py` | `assemble_system`/`build_system_prompt`；删旧 `SYSTEM_PROMPT`/`PLAN_MODE_REMINDER`/`EXECUTE_DIRECTIVE` 常量（迁移）；保留 banner |
| 新建 | `src/koyocode/prompt/environment.py` | `Environment` 结构、`gather_environment`、`Environment.render` |
| 新建 | `src/koyocode/prompt/reminder.py` | `system_reminder` 标签包裹、`plan_reminder(full)`、规划提醒完整/精简常量、`EXECUTE_DIRECTIVE` |
| 新建 | `tests/test_prompt.py` | 装配顺序、跳空槽、N1 确定性、双重强化文本断言 |
| 改   | `src/koyocode/tool/edit_file.py` | `DESCRIPTION` 补「编辑前先 `read_file`」 |
| 改   | `src/koyocode/tool/bash.py` | `DESCRIPTION` 补「优先用专用工具而非 bash 拼凑」 |
| 改   | `src/koyocode/llm/__init__.py` | `System` / `Request` dataclass；`Usage` 加缓存字段；`Provider.stream(req)`；删 `_effective_system` 与 prompt import |
| 改   | `src/koyocode/llm/anthropic_provider.py` | 两块 system（稳定块打断点 + env 块）、缓存用量解析、reminder 并入末条 user |
| 改   | `src/koyocode/llm/openai_provider.py` | 单条 system（stable+env 拼接）、`cached_tokens` 解析、reminder 追加尾部 user |
| 改   | `src/koyocode/agent/agent.py` | `__init__(+version)`；`run` 采集环境/装配系统；按轮次 reminder；缓存用量透传 |
...
| 改   | `src/koyocode/tui/stream.py` | `Agent(...)` 传 `self.version` |
...
4. 删除 `_effective_system` 函数与对 `koyocode.prompt` 的 import。
5. `new_provider` 工厂保持。
...
3. `pytest`（全量单测通过）。
4. （可选）`mypy src/koyocode` 通过子集检查。
5. `python -m koyocode` 能正常启动。
