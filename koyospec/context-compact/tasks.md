...
| 文件路径 | 类型 | 职责 |
|----------|------|------|
| `src/koyocode/compact/__init__.py` | 新建 | 重导出 `manage_context` / `TriggerKind` / 几个 State 类型 |
| `src/koyocode/compact/const.py` | 新建 | 全部硬编码常量 |
| `src/koyocode/compact/state.py` | 新建 | `ContentReplacementState`（含 `decide_once`）/ `CompactCircuitBreaker` / `RecoveryState` / `SessionContext` |
| `src/koyocode/compact/token.py` | 新建 | `estimate_tokens` / `usage_anchor` / `message_chars` |
| `src/koyocode/compact/layer1.py` | 新建 | `offload_and_snip` / `spill_single` / `build_preview` |
| `src/koyocode/compact/summary_prompt.py` | 新建 | `build_summary_prompt` / `serialize_conversation` / `extract_summary` / 9 部分模板 |
| `src/koyocode/compact/recovery.py` | 新建 | `build_recovery_attachment` / `render_file_block` / `render_tools_block` / `BOUNDARY_NOTICE` |
| `src/koyocode/compact/layer2.py` | 新建 | `auto_compact` / `force_compact` / `run_summary` / `summarize_once` / `ptl_retry` / `pick_recent_tail` / `group_by_user_turn` |
| `src/koyocode/compact/compact.py` | 新建 | `manage_context` / `TriggerKind` 枚举 / 编排 |
| `tests/compact/test_*.py` | 新建 | 各模块对应单测 |
| `src/koyocode/llm/__init__.py` | 修改 | 新增 `class PromptTooLongError(Exception)` 哨兵异常（`ToolDefinition` 已是导出类型，无需改动） |
| `src/koyocode/llm/anthropic_provider.py` | 修改 | 把 provider 上下文过长异常包装成 `PromptTooLongError` 并通过 `yield StreamEvent(err=...)` 投递 |
| `src/koyocode/llm/openai_provider.py` | 修改 | 同上 |
| `tests/llm/test_anthropic_provider.py` | 修改/新建 | PTL 异常包装单测 |
| `tests/llm/test_openai_provider.py` | 修改/新建 | PTL 异常包装单测 |
| `src/koyocode/conversation.py` | 修改 | 加 `# 无需显式锁`；新增 `replace_history(new_messages)` 深拷贝整体替换 |
| `tests/test_conversation.py` | 修改 | 增加 `replace_history` 用例 |
| `src/koyocode/config/config.py` | 修改 | `ProviderConfig` 追加 `context_window: int = 0` + 模块级函数 `effective_context_window(p)` |
| `src/koyocode/config/protocol_defaults.py` | 新建 | `DEFAULT_ANTHROPIC_CONTEXT_WINDOW` / `DEFAULT_OPENAI_CONTEXT_WINDOW` 协议默认值常量 |
| `tests/test_config.py` | 修改 | 4 种情况断言 + `yaml.safe_load` `.koyocode/config.yaml.example` 通过的解析测试 |
| `src/koyocode/agent/runtime.py` | 新建 | `SessionRuntime` dataclass |
| `src/koyocode/agent/event.py` | 新建 | `CompactPhase` / `CompactEvent`；`Event` dataclass 追加 `compact` 字段 |
| `src/koyocode/agent/agent.py` | 修改 | `__init__` 接受 `runtime` 关键字参数；`_stream_once` 签名改为返回 err；主循环集成 compact、ReadFile 追踪、PTL 紧急压缩、`run_force_compact`、`_run_lock` 互斥锁 |
| `tests/agent/test_agent.py` | 修改 | fake_provider 扩展 + 紧急压缩两用例 |
| `src/koyocode/tui/commands.py` | 新建 | 命令分发 + `/exit` / `/plan` / `/do` / `/compact` 处理器 + 未知命令兜底 |
| `src/koyocode/tui/app.py` | 修改 | `koyocodeApp` 新增 `runtime: SessionRuntime` 与 `agent: Agent` 字段；`__init__` 构造期一次性构造 Agent |
| `src/koyocode/tui/stream.py` | 修改 | `submit()` 内原 match 改用 `dispatch_command` |
| `tests/tui/test_tui.py` | 修改 | 5 组用例:`/compact` 路由到命令分发、`/unknown` 友好提示、迁移后 `/exit` / `/plan` / `/do` 各一组不回归 |
| `src/koyocode/cli.py` | 修改 | 启动期构造 `SessionRuntime` 注入 TUI；待 provider 选定后再注入 `context_window` |
| `scripts/smoke.py` | 修改 | 按新 Agent 构造签名传入 `SessionRuntime`（smoke 场景 `context_window=200000`） |
| `.koyocode/config.yaml.example` | 修改 | 新增 `context_window` 字段示例与注释 |
| `.gitignore` | 修改 | 追加 `.koyocode/sessions/` |

---

## T1 - 建立 compact 子包骨架与常量

- **文件**：`src/koyocode/compact/__init__.py`、`src/koyocode/compact/const.py`
- **依赖**：无
- **步骤**：
  1. 新建目录 `src/koyocode/compact/`，添加空 `__init__.py`（后续按导出需求填充 `from .compact import manage_context, TriggerKind` 等）。
...
- **验证**：`python -c "from koyocode.compact import const"` 不报错；`ruff check src/koyocode/compact/` 无告警。

## T2 - SessionContext 与目录创建

- **文件**：`src/koyocode/compact/state.py`
- **依赖**：T1
- **步骤**：
  ...
  4. 实现 `new_session_context(workspace: str) -> SessionContext`：
     ...
     - 拼接 `spill_dir = str(Path(workspace) / ".koyocode/sessions" / session_id / "tool-results")`。
     - 调 `Path(spill_dir).mkdir(parents=True, exist_ok=True)`；目录已存在不算错误。
     - 返回构造好的实例。
  5. 文件头部 import 按 `ruff format` / `isort` 规则分组（标准库、第三方、本地）。
- **验证**：`python -c "from koyocode.compact.state import new_session_context; print(new_session_context('.'))"` 输出合理；T22 增加 `test_new_session_context_rand_fail_fallback`（通过 monkeypatch 注入 `secrets.token_hex` 失败模拟）覆盖降级路径。
...
- **依赖**：T2
- **步骤**：
  1. 在 `state.py` 中追加 `ContentReplacementState` 类：`__init__` 内 `# 无需显式锁——Python asyncio 单线程事件循环保证串行`、`self._seen_ids: set[str] = set()`、`self._replacements: dict[str, str] = {}`。属性以 `_` 开头不导出。
  2. 给类加唯一高层方法：
     ...
     注意 `_seen_ids` 与 `_replacements` 两本结构的写入必须在同一临界区内完成（串行执行期间），避免出现"已 Seen 但 replacement 未写"的中间态。
  3. 定义 `CompactCircuitBreaker` 类：`__init__` 内 `# 无需显式锁——Python asyncio 单线程事件循环保证串行`、`self._consecutive_failures = 0`。
  4. 实现 `record_success()` / `record_failure()` / `tripped() -> bool`（`>= MAX_CONSECUTIVE_AUTO_COMPACT_FAILURES`），全部加锁。
- **验证**：`ruff check src/koyocode/compact/state.py` 干净；目测逻辑正确无并发冲突；`decide_once` 是临界区入口。
...
- **步骤**：
  ...
  2. 定义 `RecoveryState` 类：`__init__` 内 `# 无需显式锁——Python asyncio 单线程事件循环保证串行`、`self._files: dict[str, FileReadRecord] = {}`（键为绝对路径）。
...
## T5 - estimate_tokens 与 usage_anchor

- **文件**：`src/koyocode/compact/token.py`
- **依赖**：T1
- **步骤**：
...
- **文件**：`src/koyocode/compact/layer1.py`
- **依赖**：T2
- **步骤**：
...
- **验证**：`ruff check src/koyocode/compact/layer1.py` 干净；手工写一个脚本测一次重复落盘，第二次 `st_mtime_ns` 不变（也可留到 T22 测试覆盖）。

## T7 - 预览体构造 build_preview
...
- **步骤**：
  ...
  3. 关键约束：仅遍历 `msg.role == "tool"` 的消息（koyocode 把一轮工具结果挂在 tool 消息的 `tool_results` 列表里，**不**在 assistant 消息里）。对每条 tool 消息独立处理其 `tool_results` 列表。
...
- **文件**：`src/koyocode/compact/summary_prompt.py`
- **依赖**：T1
- **步骤**：
...
- **文件**：`src/koyocode/compact/recovery.py`
- **依赖**：T4
- **步骤**：
  1. 新建 `recovery.py`。
...
## T11 - build_recovery_attachment 三段拼接
...
## T12 - 近期原文 pick_recent_tail + 配对修正

- **文件**：`src/koyocode/compact/layer2.py`
- **依赖**：T5
- **步骤**：
  1. 新建 `layer2.py`。
...
## T12a - pick_recent_tail 起点 role 衔接修正
...
## T17 - auto_compact 与 force_compact
...
## T18 - manage_context 编排入口

- **文件**：`src/koyocode/compact/compact.py`
- **依赖**：T8、T17
- **步骤**：
  1. 新建 `compact.py`。
  ...
  4. 实现 `async def manage_context(in_: ManageInput) -> ManageOutput`：
     - **MANUAL 分支**：跳过 layer1、阈值、熔断；直接 `force_compact`；成功后 `in_.conv.replace_history(new_messages)`；返回 `ManageOutput(before_tokens=in_.estimated_token, after_tokens=after_tok)`。
     - **EMERGENCY 分支**：
       - 先强制跑一次 `offload_and_snip(in_.conv.messages(), in_.replacement, in_.session)` 把大工具结果挪走，`in_.conv.replace_history(layer1_out)`。
       - 再调 `force_compact`；写回 conversation；返回 `before_tokens`/`after_tokens`。
     - **AUTO 分支**：
       - a. `layer1_out = offload_and_snip(in_.conv.messages(), in_.replacement, in_.session)`；`in_.conv.replace_history(layer1_out)`（无论是否触发 layer2 都必须写回，否则 layer1 的替换不会作用到下一次 `_stream_once`）。
       ...
       - e. 否则 `await auto_compact(in_)`；成功后 `in_.conv.replace_history(new_messages)`；返回 `ManageOutput(before_tokens=in_.estimated_token, after_tokens=after_tok)`。
- **验证**：`ruff check src/koyocode/compact/` 干净；`pytest tests/compact/test_compact.py` 通过。

## T19 - llm.PromptTooLongError 哨兵

- **文件**：`src/koyocode/llm/__init__.py`
- **依赖**：无
- **步骤**：
...
- **验证**：`python -c "from koyocode.llm import PromptTooLongError"` 不报错；`grep -n PromptTooLongError src/koyocode/llm/__init__.py` 命中。

## T20 - Anthropic / OpenAI provider PTL 错误包装

- **文件**：`src/koyocode/llm/anthropic_provider.py` / `src/koyocode/llm/openai_provider.py`
- **依赖**：T19
- **步骤**：
...
- **验证**：`ruff check src/koyocode/llm/` 干净。

## T20.5 - Anthropic / OpenAI provider PTL 错误包装单测

- **文件**：`tests/llm/test_anthropic_provider.py` / `tests/llm/test_openai_provider.py`
- **依赖**：T20
- **步骤**：
...
## T21 - conversation.replace_history + 并发安全

- **文件**：`src/koyocode/conversation.py`
- **依赖**：无
- **步骤**：
  1. 给 `Conversation` 类新增 `# 无需显式锁——Python asyncio 单线程事件循环保证串行` 字段（当前 conversation.py 没有 lock）。
  ...
  3. 新增 `replace_history(self, msgs: list[Message]) -> None`：
     - `with self._lock:`。
     - 深拷贝：`new_list = copy.deepcopy(msgs)`；不暴露原入参引用。
     - 直接 `self._messages = new_list`。
- **验证**：`ruff check src/koyocode/conversation.py` 干净；T23 加并发用例覆盖 `_lock`。

## T22 - compact 包单元测试

- **文件**：`tests/compact/test_state.py` / `test_layer1.py` / `test_summary_prompt.py` / `test_recovery.py` / `test_token.py` / `test_layer2.py` / `test_compact.py` / `conftest.py`
- **依赖**：T1~T18
- **步骤**：
  1. **conftest.py**（pytest fixture 文件）：定义 `fake_compact_provider` fixture——脚本化驱动 `list[list[StreamEvent]]`，支持按调用次数 yield 不同异常（覆盖 `PromptTooLongError` 与其他异常）；在最后一帧之前 yield Usage 事件。compact 包内部测试统一用这个 helper。**不要** import `koyocode.agent` 的 fake_provider（位于 `tests/agent/`，作用域不重合）。
  2. `test_state.py`：
...
## T23 - conversation.replace_history 测试

- **文件**：`tests/test_conversation.py`
- **依赖**：T21
- **步骤**：
  1. 新增 `test_replace_history_deep_copy`：构造 2 条 msg，调 `replace_history`，修改原列表后 `conv.messages()` 不被影响。
  2. 新增 `test_replace_history_empty`：传 `None` / 空列表不抛异常，`messages()` 返回长度 0（注意 `replace_history(None)` 是否允许：实现里可 `msgs = msgs or []`）。
  3. 风格遵循文件内已有断言风格（直接 `assert ... == ...`，不引入 parametrize）。
- **验证**：`pytest tests/test_conversation.py` 通过。

## T24 - config Provider 新增 context_window 与默认值

- **文件**：`src/koyocode/config/config.py` + `src/koyocode/config/protocol_defaults.py`（新建）
- **依赖**：T1
- **步骤**：
  1. 新建 `src/koyocode/config/protocol_defaults.py`：
...
- **验证**：`ruff check src/koyocode/config/` 干净；`grep -n compact src/koyocode/config/` 无命中（config 不 import compact）。
...
- **文件**：`src/koyocode/agent/runtime.py`（新建）+ `src/koyocode/agent/agent.py`（修改）
- **依赖**：T2、T3、T4、T19、T24
- **步骤**：
  1. 新建 `src/koyocode/agent/runtime.py`：
     ...
     from koyocode.compact import (
         ...
         CompactCircuitBreaker, SessionContext,
     )
  ...
  2. 修改 `Agent` 类：
     ...
     - 新增 `# 用 asyncio.Lock 保证 run 与 force_compact 不并发`。
  ...
  3. 调整 `_stream_once` 签名为 `(text: str, calls: list[ToolCall], usage: Usage | None, err: Exception | None)`：
     ...
     - 注意累加的 text 在 err 路径下**不**写回 Conversation（保持原子，紧急压缩可安全 `replace_history`）。
- **验证**：`ruff check src/koyocode/agent/` 干净。

## T27 - Agent 主循环集成 manage_context

- **文件**：`src/koyocode/agent/agent.py`
- **依赖**：T18、T21、T26
- **步骤**：
...
- **步骤**：
  ...
  2. 判断条件：`calls[i].name == "read_file"`（koyocode 现有工具名以 `src/koyocode/tool/read_file.py` 的 `name` 属性为准）且 `tool.Result.is_error is False`（`is_error` 来自 `tool.Result`，不是 `llm.ToolResult`）。
  3. 从 `ToolCall.input`（dict）取参数：
     ...
     注意 `ToolCall` 字段在 Python 适配后是 `input` 而非 `arguments`，参数名以 `src/koyocode/tool/read_file.py` 定义的 schema 为准（koyocode 现在用 `path`）。
...
- **验证**：`ruff check src/koyocode/agent/` 干净；自查只对 ReadFile 工具触发，其他工具不会写 recovery。
...
- **文件**：`src/koyocode/agent/event.py`（新建）+ `src/koyocode/agent/agent.py`、`tests/agent/test_agent.py`
- **依赖**：T27、T29
- **步骤**：
  1. 新建 `src/koyocode/agent/event.py`：
...
## T30 - Agent run_force_compact 给 TUI 调
...
## T32 - cli 与 smoke 入口改造

- **文件**：`src/koyocode/cli.py` + `scripts/smoke.py`
- **依赖**：T2、T3、T4、T26
- **步骤**：
  1. 启动阶段构造：
  ...
  2. `src/koyocode/cli.py`：若 `providers` 长度 == 1，启动期即可知 protocol → 直接计算 `cw = effective_context_window(providers[0])`，构造 `runtime = SessionRuntime(...)` 注入 `koyocodeApp`。若 `len(providers) > 1`，让 TUI 完成 provider 选择后再注入 `context_window`（在 TUI 的 provider-selected 回调里调 `runtime.context_window = effective_context_window(provider_cfg)`）。
...
- **验证**：`python -m koyocode --help` 可启动；启动后 `.koyocode/sessions/<id>/tool-results/` 目录存在。

## T32.5 - TUI Model 持有 SessionRuntime 与 Agent 引用

- **文件**：`src/koyocode/tui/app.py` + `src/koyocode/tui/stream.py`
- **依赖**：T26、T32
- **步骤**：
  1. `koyocodeApp` 类新增字段 `runtime: SessionRuntime` 与 `agent: Agent`。
...
- **验证**：`ruff check src/koyocode/tui/` 干净。

## T33 - TUI 命令分发框架

- **文件**：`src/koyocode/tui/commands.py`
- **依赖**：T30、T32.5
- **步骤**：
  1. 新建 `commands.py`。
  2. 定义 `CommandHandler = Callable[["koyocodeApp"], Awaitable[None]]`。
...
## T34 - TUI 输入路径接入 dispatch_command

- **文件**：`src/koyocode/tui/stream.py`
- **依赖**：T33
- **步骤**：
  1. koyocode 现有 `src/koyocode/tui/stream.py` 的 `submit()` 内部已有针对 `/exit` / `/plan` / `/do` 的 match 分支。把这段 match 整段删除，改为统一的命令分发：
...
- **验证**：`ruff check src/koyocode/tui/` 干净；手动跑 `tmux` 启动 koyocode 输入 `/compact`、`/exit`、`/plan`、`/do` 验证行为；输入 `/unknown` 看到友好提示。

## T34a - TUI 渲染 Compact 状态事件（兑现 spec F24a / F24b）

- **文件**：`src/koyocode/tui/stream.py`、`src/koyocode/tui/commands.py`、`tests/tui/test_tui.py`
- **依赖**：T29a、T34
- **步骤**：
...
- **文件**：`.koyocode/config.yaml.example`
- **依赖**：T24
- **步骤**：
...
- **验证**：编写小测试 fixture，`yaml.safe_load(Path(".koyocode/config.yaml.example").read_text())` 解析后通过 `_from_dict` 转为 `Config` 不报错。
...
- **依赖**：T2
- **步骤**：
  1. 在 `.gitignore` 末尾追加一行 `.koyocode/sessions/`。
  2. 提交时在 PR 说明中点出。
- **验证**：启动 koyocode 后跑一次工具调用，`git status` 干净（不出现 sessions 子目录）。
...
- **步骤**：
  ...
  2. 用 tmux 启动 `python -m koyocode`，配置好 anthropic provider。
  3. 触发一次大文件 ReadFile（构造一个 80KB 文件）→ 观察 `.koyocode/sessions/<id>/tool-results/` 下出现该工具调用 id 的文件；下一轮请求里该工具结果展示成预览体。
...
- T21（replace_history）独立，可与 T1~T20 并行。
