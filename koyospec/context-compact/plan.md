...
ch08 引入一个新的本地子包 `koyocode.compact`，作为上下文管理的唯一权威入口。包内承担三块职责：

1. **第 1 层预防性压缩**：在每一轮 LLM 请求发出之前，对 `koyocode.conversation` 中的工具结果做幂等的"超阈值落盘 + 字符串替换"，并把替换决策冻结在一个会话级账本里，保证 prompt cache 前缀逐字节稳定。
...
`koyocode.compact` 不直接持有 `Agent`,也不直接管理 `Provider`。它通过一组窄接口与外部模块交互：

| 外部模块 | 交互方向 | 形式 |
|----------|----------|------|
| `koyocode.agent` | Agent 调 compact | 主循环每轮请求前调 `manage_context`；ReadFile 成功后调 `RecoveryState.record_file`；捕获 `PromptTooLongError` 后调 `force_compact` 重试一次 |
| `koyocode.conversation` | compact 改 conversation | compact 拿到 `list[Message]` 后做字符串替换 / 摘要重建，再用一个新方法 `replace_history` 整体替换内存列表 |
| `koyocode.llm` | compact 调 provider | 摘要请求复用同一份 `Provider.stream`，但 `Request.tools` 留空；从 `StreamEvent` 尾部拿 usage 锚定 token 估算 |
| `koyocode.tui` | TUI 调 compact | TUI 拿到以 `/` 开头的输入走命令分发；`/compact` 命令调 compact 的 `force_compact` 并展示 token 变化系统消息 |
| `koyocode.config` | config 喂 compact | `ProviderConfig` 新增 `context_window: int`，未配置时按协议给默认值；compact 通过参数拿到当前 provider 的 `context_window` |

**Agent 生命周期与状态归属调整**：现状的 TUI 在 `_begin_turn` 里每轮 `Agent(...).run(...)` 重新构造一次 Agent（见 `src/koyocode/tui/stream.py`），意味着把 compact 的长生命周期状态（替换决策账本、文件追踪、自动摘要熔断计数、`usage_anchor`、本轮工具列表缓存）放成 `Agent` 字段会被每轮重置——决策冻结与熔断器立刻失效。

本章引入 `SessionRuntime` 作为 TUI Model 跨 run 持有的长生命周期状态容器：

```python
# src/koyocode/agent/runtime.py（新建）
...
class SessionRuntime:
    ...
    # asyncio 单线程，无需锁  # 保护 usage_anchor / anchor_msg_len
```
...
**依赖方向无环**：
- `koyocode.compact` 不 import `koyocode.agent` / `koyocode.config` / `koyocode.tui` / `koyocode.cli`。
- `koyocode.config` 仅在 `effective_context_window()` 中读自身常量（`DEFAULT_ANTHROPIC_CONTEXT_WINDOW` / `DEFAULT_OPENAI_CONTEXT_WINDOW` 定义在 `src/koyocode/config/protocol_defaults.py`，不放 compact 子包）。
- `koyocode.agent` 依赖 `koyocode.compact` + `koyocode.conversation` + `koyocode.llm` + `koyocode.tool` + `koyocode.permission`，**不** import `koyocode.config`。
- `src/koyocode/cli.py` 是唯一同时 import `config` 与 `agent` 的位置，负责把 `provider_cfg.effective_context_window()` 注入 `SessionRuntime`。
- `koyocode.tui` 持有 `SessionRuntime` 与 `Agent`（或在每轮构造 Agent 时把 runtime 传入）。

## 核心数据结构

```python
# src/koyocode/compact/state.py
...
#
# 并发安全约束:offload_and_snip 在执行期间在 asyncio 事件循环中串行执行(读账本 → 决策 → 落盘 →
...
# CompactCircuitBreaker 跟踪自动摘要连续失败次数,用于熔断。
# 手动 / 紧急压缩路径不读这个字段。
...
# spill_dir 是落盘目录，固定指向 .koyocode/sessions/<session_id>/tool-results/。
@dataclass
...
```python
# src/koyocode/config/config.py 改动(仅追加字段,不动现有字段顺序与名字)
...
# src/koyocode/config/protocol_defaults.py（新文件）
...
> **依赖方向说明**：协议默认值常量定义在 `src/koyocode/config/protocol_defaults.py`，由 `koyocode.config` 自身使用；`koyocode.compact` 包不持有协议默认值常量。`config` 与 `compact` 单向无环。

## 模块设计

### compact 子包

#### `compact.py` - manage_context 主入口
...
async def manage_context(in_: ManageInput) -> ManageOutput:
    ...
    步骤:
      ...
      2. 否则(AUTO 路径):
         ...
         d. 否则 auto_compact, 成功后 replace_history。
...
def offload_and_snip(
    ...
    (koyocode 在 Conversation.add_tool_results 把一轮工具结果挂在一条 RoleTool
...
依赖：`koyocode.llm.Provider`、`summary_prompt`、`recovery`、`token`、`CompactCircuitBreaker`。
...
依赖：`FileReadRecord`、`koyocode.llm.ToolDefinition`。
...
### Agent 主循环改造（`src/koyocode/agent/agent.py`）
...
class Agent:
    def __init__(
        ...
        *,
        ...
        # 用 asyncio.Lock 保证 run 与 force_compact 不并发                # 保证 run 与 run_force_compact 不并发
```
...
3. **`_stream_once` 签名扩展为返回 (text, calls, usage, err)**：把现有 `(text, calls, usage, ok)` 改成 `(text, calls, usage, err: Exception | None)`。错误来源是 `StreamEvent.err`（koyocode 的 `Provider.stream` 是 async generator，错误通过事件流投递）。`_stream_once` 在收到 ev.err 时累加的 text 不写回 Conversation（保证 Conversation 状态与 stream 调用前一致，紧急压缩可以安全地 `replace_history`）。
...
> **压缩状态事件 emit**（兑现 spec F24a / F24b）：`Event` 结构新增 `compact: CompactEvent | None` 字段（`src/koyocode/agent/event.py`）。主循环在以下两个时机向事件队列 emit 状态事件，让 TUI 在 LLM 摘要请求还在跑的时候就能立刻显示"压缩中"前缀，避免用户以为程序卡死：
>
...
### Conversation 改造（`src/koyocode/conversation.py`）

新增一个整体替换方法，并补充内部 lock 保护：

```python
# Conversation 内部新增 # 无需显式锁
# messages / replace_history / add_xxx 都加锁,
# 防止 replace_history 与 messages 并发时拿到部分写入的列表。
...
> **性能评估**：每轮 `manage_context` 都会调 `replace_history`（layer1-only 时也要写回，否则 `offload_and_snip` 的字符串替换不会作用于下一轮）。25 轮 × 数十条消息 × 数百 KB 字符串的深拷贝在毫秒级完成（CPython 3.12 `copy.deepcopy` 性能足够），与摘要 LLM 请求几十秒耗时相比可忽略；不做对象池。

### TUI 命令分发（`src/koyocode/tui/`）

`src/koyocode/tui/stream.py` 现有 `submit()` 内部已经有针对 `/exit` / `/plan` / `/do` 的 `match/case` 分支。本章把这三个命令一并迁移到统一注册表，并新增 `/compact`：

```python
# src/koyocode/tui/commands.py(新文件)

from collections.abc import Awaitable, Callable
from typing import Any

CommandHandler = Callable[[Any], Awaitable[None]]   # 入参为 koyocodeApp
...
**TUI 渲染 Compact 事件**（兑现 spec F24a / F24b）：`src/koyocode/tui/stream.py` 的 `_update_streaming` 在 `StreamMsg` 处理上新增 `msg.compact is not None` 分支，按 phase 渲染系统消息后继续 `await self.events.__anext__()` 拉下一帧，**不写入 conversation**：
...
### `.koyocode/config.yaml.example` 更新
...
Agent.run() asyncio task
    ...
    │   │  │    e. 拼接成 new_msgs                    │   │
    │   │  │    f. conversation.replace_history      │   │
    │   │  │    g. 成功→失败计数清零；失败→+1，熔断   │   │
    │   │  └──────────────────────────────────────────┘   │
    │   └─────────────────────────────────────────────────┘
    │
...
manage_context(trigger=EMERGENCY)
    ...
    │   - 再 force_compact → run_summary → replace_history
...
├── state.py           — ContentReplacementState (decide_once) / CompactCircuitBreaker / SessionContext
└── const.py           — 全部硬编码常量
...
`src/koyocode/agent/agent.py` 改动：
...
`src/koyocode/agent/runtime.py`（新文件）：定义 `SessionRuntime` dataclass 与构造工厂。

`src/koyocode/agent/event.py`（新文件或修改 `__init__.py`）：定义 `CompactPhase`、`CompactEvent`；`Event` dataclass 追加 `compact: CompactEvent | None = None`。
...
`src/koyocode/conversation.py` 改动：新增 `# 无需显式锁`；新增 `replace_history(new_messages)`，做深拷贝；已有 `add_xxx` / `messages` / `length` / `last_role` 全部加锁。

`tests/test_conversation.py` 改动：新增 `replace_history` 的直接断言用例。

`src/koyocode/llm/__init__.py` 改动：
- 新增 `class PromptTooLongError(Exception)` 哨兵异常。
- `ToolDefinition` 已是导出类型，无需改动。

`src/koyocode/llm/anthropic_provider.py` / `src/koyocode/llm/openai_provider.py` 改动：捕获 provider SDK 抛出的"上下文过长"错误（`anthropic.BadRequestError` 且 message 含 `prompt is too long`；`openai.BadRequestError` 且 `code == "context_length_exceeded"`），包装成 `PromptTooLongError(orig)` 并通过 `yield StreamEvent(err=wrapped)` 投递（async generator 错误走事件流，不直接 raise）。
...
`src/koyocode/tui/commands.py`（新文件）：`dispatch_command` + `handle_exit` / `handle_plan` / `handle_do` / `handle_compact` + 未知命令兜底。
`src/koyocode/tui/stream.py`：`submit()` 内原 `match/case` 改用 `dispatch_command` 调用；命令路径不调 `conv.add_user`，不写入对话历史。
`src/koyocode/tui/app.py`：`koyocodeApp` 新增 `runtime: SessionRuntime` 与 `agent: Agent` 字段；`__init__` 构造期一次性构造 Agent 并保存。
`tests/tui/test_tui.py`：① `/compact` 走命令路径不发 LLM；② `/unknown` 友好提示；③ 迁移后 `/exit` / `/plan` / `/do` 行为不回归三个用例。

`src/koyocode/config/config.py`：`ProviderConfig` 追加 `context_window: int = 0`，加 `effective_context_window(p)`；现有字段顺序与 yaml 键名不变。
`src/koyocode/config/protocol_defaults.py`（新文件）：`DEFAULT_ANTHROPIC_CONTEXT_WINDOW = 200000`、`DEFAULT_OPENAI_CONTEXT_WINDOW = 128000`。
`tests/test_config.py`：新增四种情况断言。

`src/koyocode/cli.py`：启动阶段调 `new_session_context(workspace)`、`ContentReplacementState()`、`RecoveryState()`、`CompactCircuitBreaker()`，组装为 `SessionRuntime`；待 provider 选定后注入 `effective_context_window(p)`；把 `SessionRuntime` 交给 `koyocodeApp`。

`scripts/smoke.py`（若存在）：同样按新签名构造 Agent；smoke 场景的 `context_window` 可固定 200000。

`.gitignore`：追加 `.koyocode/sessions/`，避免开发者跑一次 koyocode 后 `git status` 出现一大坨 session 子目录。

`.koyocode/config.yaml.example`：新增 `context_window` 字段示例与注释。

## 技术决策

| 决策点 | 选择 | 理由 |
|--------|------|------|
| 包结构与命名 | `koyocode.compact` 单子包，子模块按职责拆分（layer1 / layer2 / recovery / token） | 上下文管理逻辑高度内聚，对外只暴露 `manage_context` 等少量函数。单子包简化导入路径，多模块保证可读性。再拆 sub-sub-package 会引入循环引用风险（layer2 既要 token 又要 recovery）。 |
...
| `CompactCircuitBreaker` 独立于 `ContentReplacementState` | 拆成两个 class | 熔断只用于自动路径，手动 / 紧急完全绕过；放一起会让"是否应该读熔断字段"在调用点变得模糊。两个 class 都内嵌 `# 无需显式锁` 保证并发安全。 |
...
| ReadFile 后用 `Path.read_bytes` 重读纯净字节 | 工具 worker task 内同步 `await asyncio.to_thread(...)` | 工具返回字符串带行号前缀（koyocode 现有实现），直接拿来做恢复段会让模型把行号当成代码的一部分。重读一次磁盘成本可忽略；同步顺序保证下一次 `manage_context` 能观察到本轮记录。 |
...
| `session_id` 不持久化 | 进程启动生成 `<unix_ts>-<short_random>`（`secrets.token_hex(4)`） | 单进程会话边界等于进程边界，不需要恢复。`.koyocode/sessions/` 留作调试副产物，外部脚本/用户决定清理时机。 |
...
| Conversation 内部 `RLock` + `replace_history` 深拷贝 | 加锁 + `copy.deepcopy(msgs)` | 摘要后 compact 把 `new_msgs` 交出去就不应该再被外部改动；Conversation 也不应该暴露内部列表引用。深拷贝在 25 轮 × 数百 KB 量级下耗时毫秒级，与摘要 LLM 请求耗时相比可忽略；不再做对象池。 |
...
| `context_window` 注入时机 | provider 选定后由 `koyocode.cli` 注入 `SessionRuntime`，本会话内不变 | koyocode 启动期 TUI 可能选 provider，等用户选定后才能确定 `context_window`；本章不支持运行期切 provider；切换 provider 等同于重新启动进程。 |
